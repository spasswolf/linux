...
IPA_REG_FIELDS(COMP_CFG, comp_cfg, 0x00000038);

IPA_REG(COMP_SW_RESET, comp_sw_reset, 0x0000003c);
...
IPA_REG_FIELDS(ROUTE, route, 0x00000044);
...
IPA_REG_FIELDS(SHARED_MEM_SIZE, shared_mem_size, 0x00000050);

IPA_REG(IPA_BCR, ipa_bcr, 0x000005b0);

IPA_REG(ENABLED_PIPES, enabled_pipes, 0x000005dc);
...
IPA_REG_FIELDS(COUNTER_CFG, counter_cfg, 0x000005e8);
...
IPA_REG_STRIDE_FIELDS(ENDP_INIT_CTRL, endp_init_ctrl, 0x00000070, 0x0004);
...
IPA_REG_STRIDE_FIELDS(ENDP_INIT_CFG, endp_init_cfg, 0x000000c0, 0x0004);
...
IPA_REG_STRIDE_FIELDS(ENDP_INIT_NAT, endp_init_nat, 0x00000120, 0x0004);
...
IPA_REG_STRIDE_FIELDS(ENDP_INIT_HDR, endp_init_hdr, 0x00000170, 0x0004);
...
IPA_REG_STRIDE_FIELDS(ENDP_INIT_HDR_EXT, endp_init_hdr_ext, 0x000001c0, 0x0004);

IPA_REG_STRIDE(ENDP_INIT_HDR_METADATA_MASK, endp_init_hdr_metadata_mask, 0x00000220, 0x0004);
...
IPA_REG_STRIDE_FIELDS(ENDP_INIT_MODE, endp_init_mode, 0x000002c0, 0x0004);
...
IPA_REG_STRIDE_FIELDS(ENDP_INIT_AGGR, endp_init_aggr, 0x00000320, 0x0004);
...
IPA_REG_STRIDE_FIELDS(ENDP_INIT_HOL_BLOCK_EN, endp_init_hol_block_en, 0x000003c0, 0x0004);
...
IPA_REG_STRIDE_FIELDS(ENDP_INIT_HOL_BLOCK_TIMER, endp_init_hol_block_timer, 0x00000420, 0x0004);
...
IPA_REG_STRIDE_FIELDS(ENDP_INIT_DEAGGR, endp_init_deaggr, 0x00000470, 0x0004);
...
IPA_REG_STRIDE_FIELDS(ENDP_STATUS, endp_status, 0x000004c0, 0x0004);

IPA_REG(IPA_IRQ_STTS, ipa_irq_stts, 0x00001008 + 0x1000 * DMA_EE_AP);

IPA_REG(IPA_IRQ_EN, ipa_irq_en, 0x0000100c + 0x1000 * DMA_EE_AP);

IPA_REG(IPA_IRQ_CLR, ipa_irq_clr, 0x00001010 + 0x1000 * DMA_EE_AP);

IPA_REG(IRQ_SUSPEND_INFO, irq_suspend_info, 0x00001098 + 0x1000 * DMA_EE_AP);

static const struct ipa_reg *ipa_reg_array[] = {
	[COMP_CFG]			= &ipa_reg_comp_cfg,
	[COMP_SW_RESET]			= &ipa_reg_comp_sw_reset,
	[ROUTE]				= &ipa_reg_route,
	[SHARED_MEM_SIZE]		= &ipa_reg_shared_mem_size,
	[IPA_BCR]			= &ipa_reg_ipa_bcr,
	[ENABLED_PIPES]			= &ipa_reg_enabled_pipes,
...
const struct ipa_regs ipa_regs_v2_6l = {
